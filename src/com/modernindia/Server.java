...
public class Server extends JFrame{
    ...
    Server(){
        ...
        setLocation(200,100);   // location of chat window in screen
...
/*  NOTE:
    ...
    */
        ...
        addMyIcon("icons/black-panther.png",60,60);
        label.setBounds(40,0,60,60);
        panel.add(label);

        // building display Name label
        JLabel name = new JLabel("B PANTHER");
        name.setFont(new Font("SAN_SERIF", Font.BOLD,20));
        name.setForeground(Color.white);
        name.setBounds(110,10,120,20);
        panel.add(name);
        ...
        panel.add(status);
        Timer timer = new Timer(1, event -> {   // to set time till, typing or not
            if(!typing){
...
//            chatArea.setText(chatArea.getText()+"\n\t\t"+message); //get text from chat area(all previous)
    ...
    private void backUpFile(String message) {
        try {
            ...
            printWriter.println("HULK: "+message);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
